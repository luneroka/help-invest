...
export default function TransactionForm({ title, actionType, onSubmit, isLoading }) {
    ...
    const fetchCategories = async () => {
        try {
            ...
            const mockCategories = [
                ...
                { category_name: 'Immobilier', sub_category: 'Crowdfunding Immobilier' },
    ...
    const validateForm = () => {
        ...
        if (actionType === 'withdraw' && formData.amount && formData.categoryName && formData.subCategory) {
    ...
    const currentBalance = formData.categoryName && formData.subCategory ?
        availableBalances[`${formData.categoryName}-${formData.subCategory}`] : null;

    return (
        <div className="card">
            <h2 className={title === 'Investir' ? 'text-alerts-success text-center' : 'text-alerts-error text-center'}>{title}</h2>

            <form onSubmit={handleSubmit} className="flex flex-col gap-8">
                {/* Category Selection */}
                <div className="flex flex-col gap-2">
                    <label htmlFor={`category-${actionType}`} className="block text-small">
                        Catégorie
                    </label>
                    <select
                        name="categoryName"
                    ...
                    {errors.categoryName && (
                        <span className="text-small text-red-500">{errors.categoryName}</span>
                    )}
                </div>

                {/* Sub-category Selection */}
                <div className="flex flex-col gap-2">
                    <label htmlFor={`subcategory-${actionType}`} className="block text-small">
                        Compte
                    </label>
                    <select
                        name="subCategory"
                    ...
                    {errors.subCategory && (
                        <span className="text-small text-red-500">{errors.subCategory}</span>
                    )}
                </div>

                {/* Current Balance Display (for withdrawals) */}
                {isWithdraw && currentBalance !== null && currentBalance !== undefined && (
                    <div className="p-3 bg-gray-100 rounded-lg">
                        <span className="text-small text-gray-600">
                            Solde disponible: <span className="font-medium text-data">{formatNumber(currentBalance)}€</span>
                        </span>
                    </div>
                )}
