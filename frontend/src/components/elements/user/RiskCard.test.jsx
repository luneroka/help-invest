...
describe('RiskCard', () => {
  ...
  it('renders select options and updates risk profile on change', async () => {
    ...
    const select = await screen.findByRole('combobox', {
      name: /modifiez votre profil de risque/i,
    });
  ...
  it('submits the form and updates current profile', async () => {
    ...
    await waitFor(() => {
      const profileContainer = screen.getByText(
        /votre profil de risque actuel/i
      ).parentElement;
      expect(
        within(profileContainer).getByText(/DYNAMIQUE/i)
      ).toBeInTheDocument();
    });
  });
  ...
  it('displays error message if API submission fails', async () => {
    ...
    expect(
      await screen.findByText(/erreur de mise à jour/i)
    ).toBeInTheDocument();
  });
});
